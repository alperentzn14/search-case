...
const ResultSearchInput = () => { /** States */
  ...
  const currentData = Data.slice( /** slice data kullanarak - Kaç personel görüneceğini belirliyoruz */
    (currentPage - 1) * LIMIT,
    (currentPage - 1) * LIMIT + LIMIT
  );


  const sortedasc = Data.sort((a, b) => { /** sort data kullanarak - Verileri isime göre sıralama işlemi  */
  ...
  return (
    <div>
      ...
      <form className="result-form-list">
        <ul>
          {currentData
            .filter((item) => { /** add search filter - arama filtresini burada eklendi */
            ...
            .map((item) => {  /** map json data - Json verilerini map ile döndürüldü */
              return (
